...
// Use direct mapping
class DllAvFormat : public DllAvFormatInterface
{
	public:
		virtual ~DllAvFormat() {}
		...
		virtual int url_feof(AVIOContext *s)
		{
			return ::url_feof(s);
		}
		...
		{
			*score_max = 100;    // Use av_probe_input_format, this is not exported by ffmpeg's headers
			return ::av_probe_input_format(pd, is_opened);
		}
		...
		{
			return ::av_probe_input_buffer(pb, fmt, filename, logctx, offset, max_probe_size);
		}
		...
		virtual AVFormatContext *avformat_alloc_context()
		{
		...
		virtual int avformat_network_init  (void)
		{
			return ::avformat_network_init();
		}
		virtual int avformat_network_deinit  (void)
		{
