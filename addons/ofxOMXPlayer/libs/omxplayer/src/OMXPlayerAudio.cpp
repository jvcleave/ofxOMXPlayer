...
*/
      ...
      if (fabs(error) > limit - 0.001)
      {
        ...
        /*
        if(m_speed == DVD_PLAYSPEED_NORMAL)
          ofLog(OF_LOG_VERBOSE, "CDVDPlayerAudio:: Discontinuity - was:%f, should be:%f, error:%f", clock, clock+error, error);
        */
      }
    }
/*
    ...
    if (m_skipdupcount == 0 && fabs(m_error) > DVD_MSEC_TO_TIME(10))
    {
      ...
      if (m_skipdupcount > 0)
        ofLog(OF_LOG_VERBOSE, "OMXPlayerAudio:: Duplicating %i packet(s) of %.2f ms duration",
                  m_skipdupcount, duration / DVD_TIME_BASE * 1000.0);
      else if (m_skipdupcount < 0)
        ofLog(OF_LOG_VERBOSE, "OMXPlayerAudio:: Skipping %i packet(s) of %.2f ms duration ",
