...
bool OMXClock::OMXSetReferenceClock(bool has_audio, bool lock /* = true */)
{
  ...
  if (refClock.eClock != m_eClock)
  {
    ...
    {
      CLog::Log(LOGNOTICE, "OMXClock::OMXSetReferenceClock error setting OMX_IndexConfigTimeActiveRefClock");
      ret = false;
    }
...
bool OMXClock::OMXStateExecute(bool lock /* = true */)
{
  ...
  if(m_omx_clock.GetState() != OMX_StateExecuting)
  {
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGNOTICE, "OMXClock::StateExecute m_omx_clock.SetStateForComponent\n");
      if(lock)
        UnLock();
      return false;
    }
...
bool  OMXClock::OMXStop(bool lock /* = true */)
{
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGNOTICE, "OMXClock::Stop error setting OMX_IndexConfigTimeClockState\n");
    if(lock)
      UnLock();
    return false;
  }
...
bool OMXClock::OMXStep(int steps /* = 1 */, bool lock /* = true */)
{
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGNOTICE, "OMXClock::Error setting OMX_IndexConfigSingleStep\n");
    if(lock)
      UnLock();
    return false;
  }
...
bool OMXClock::OMXReset(bool has_video, bool has_audio, bool lock /* = true */)
{
  ...
  if (m_eState == OMX_TIME_ClockStateStopped)
  {
    ...
    if(clock.nWaitMask)
    {
      ...
      if(omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGNOTICE, "OMXClock::OMXReset error setting OMX_IndexConfigTimeClockState\n");
        if(lock)
          UnLock();
        return false;
      }
...
double OMXClock::OMXMediaTime(bool lock /* = true */)
{
  ...
  if (now - m_last_media_time_read > DVD_MSEC_TO_TIME(100) || m_last_media_time == 0.0)
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGNOTICE, "OMXClock::MediaTime error getting OMX_IndexConfigTimeCurrentMediaTime\n");
      if(lock)
        UnLock();
      return 0;
    }
...
double OMXClock::OMXClockAdjustment(bool lock /* = true */)
{
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGNOTICE, "OMXClock::MediaTime error getting OMX_IndexConfigClockAdjustment\n");
    if(lock)
      UnLock();
    return 0;
  }
...
bool OMXClock::OMXMediaTime(double pts, bool lock /* = true*/)
{
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGNOTICE, "OMXClock::OMXMediaTime error setting %s", index == OMX_IndexConfigTimeCurrentAudioReference ?
...
bool OMXClock::OMXSetSpeed(int speed, bool lock /* = true */, bool pause_resume /* = false */)
{
  ...
  if (pause_resume)
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGNOTICE, "OMXClock::OMXSetSpeed error setting OMX_IndexConfigTimeClockState\n");
      if(lock)
        UnLock();
      return false;
    }
...
bool OMXClock::HDMIClockSync(bool lock /* = true */)
{
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGNOTICE, "OMXClock::Speed error setting OMX_IndexConfigLatencyTarget\n");
    if(lock)
      UnLock();
    return false;
  }
