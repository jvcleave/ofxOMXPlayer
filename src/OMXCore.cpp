...
bool COMXCoreComponent::Initialize( const std::string &component_name, OMX_INDEXTYPE index, OMX_CALLBACKTYPE *callbacks)
{
  ...
  {
      printf("%s", "USING CUSTOM FillBufferDone");
  ...
  OMX_BUFFERHEADERTYPE* pBuffer)
{
  if(!pAppData)
  {
    ...
    if(ctx->fillBufferListener)
    {
        //printf("%s", "HAS fillBufferListener");
        ctx->fillBufferListener->onFillBuffer(hComponent, pBuffer);
    }else
    {
